...
def _islice_helper(it, s):
    ...
    if step > 0:
        ...
        if start < 0:
            ...
            n = j - i
            if n <= 0:
                return for index, item in islice(cache, 0, n, step):
                yield item
...
def ichunked(iterable, n):
    ...
    while True:
        ...
        if item is _marker:
            return # Clone the source and yield an n-length slice
...
def _chunked_even_finite(iterable, N, n):
    if N < 1:
        return # Lists are either size `full_size <= n` or `partial_size = full_size - 1`
    q, r = divmod(N, n)
...
def zip_broadcast(*objects, scalar_types=(str, bytes), strict=False):
    ...
    size = len(objects)
    if not size:
        return iterables, iterable_positions = [], []
