...
class ProgressBar(t.Generic[V]):
    ...
    def render_finish(self) -> None:
        if self.hidden or not self._is_atty:
            return self.file.write(AFTER_BAR)
        self.file.flush()
    ...
    def make_step(self, n_steps: int) -> None:
        ...
        if (time.time() - self.last_eta) < 1.0:
            ...
            return self.last_eta = time.time()
...
def pager(generator: cabc.Iterable[str], color: bool | None = None) -> None:
    ...
    if pager_cmd_parts:
        if WIN:
            if _tempfilepager(generator, pager_cmd_parts, color):
                return elif _pipepager(generator, pager_cmd_parts, color):
            return
    ...
    if (WIN or sys.platform.startswith("os2")) and _tempfilepager(
        ...
        return if _pipepager(generator, ["less"], color):
        return
    ...
    try:
        if _pipepager(generator, ["more"], color):
            return return _nullpager(stdout, generator, color)
    finally:
        os.unlink(filename)
