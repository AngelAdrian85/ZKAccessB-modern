...
class ExtClassBuilder(ClassBuilder):
    ...
    def add_attr(self, lvalue: NameExpr, stmt: AssignmentStmt) -> None:
        ...
        if isinstance(stmt.rvalue, TempNode):
            return # Only treat marked class variables as class variables.
        if not (is_class_var(lvalue) or stmt.is_final_def):
            return typ = self.builder.load_native_type_object(self.cdef.fullname)
...
def generate_attr_defaults_init(
    ...
    if not default_assignments:
        return cls = builder.mapper.type_to_ir[cdef.info]
    if cls.builtin_base:
        return with builder.enter_method(cls, "__mypyc_defaults_setup", bool_rprimitive):
