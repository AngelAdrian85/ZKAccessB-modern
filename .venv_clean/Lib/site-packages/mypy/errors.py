...
class Errors:
    ...
    def _add_error_info(self, file: str, info: ErrorInfo) -> None:
        ...
        if self._filter_error(file, info):
            return if file not in self.error_info_map:
    ...
    def add_error_info(self, info: ErrorInfo) -> None:
        ...
        if self._filter_error(file, info):
            return if not info.blocker:  # Blockers cannot be ignored
            ...
            if file in self.ignored_files:
                return if info.only_once:
            if info.message in self.only_once_messages:
                return self.only_once_messages.add(info.message)
        if (
        ...
        if (
            ...
            if message in self.only_once_messages:
                return self.only_once_messages.add(message)
            info = ErrorInfo(
                import_ctx=info.import_ctx,
                file=info.file,
                module=info.module,
    ...
    def generate_unused_ignore_errors(self, file: str) -> None:
        if (
            ...
            return ignored_lines = self.ignored_lines[file]
    ...
    def generate_ignore_without_code_errors(
        ...
        if (
            ...
            return used_ignored_lines = self.used_ignored_lines[file]
        ...
        if used_ignored_lines:
            ...
            if codes.FILE.code in used_codes:
                return for line, ignored_codes in self.ignored_lines[file].items():
            if ignored_codes:
                continue
