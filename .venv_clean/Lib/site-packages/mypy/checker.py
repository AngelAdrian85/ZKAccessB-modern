...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi):
    ...
    def visit_overloaded_func_def(self, defn: OverloadedFuncDef) -> None:
        if not self.recurse_into_functions:
            return with self.tscope.function_scope(defn):
        ...
        for i, item in enumerate(defn.items):
            ...
            if impl_type is not None:
                ...
                if is_callable_compatible(
                    ...
                    def is_compat_return(l, r):
                        return is_subtype(r, l),
                ):
                    continue
    ...
    def check_reverse_op_method(
        ...
        if isinstance(ret_type, AnyType):
            return if isinstance(ret_type, Instance):
            if ret_type.type.fullname == "builtins.object":
                return if reverse_type.arg_kinds[0] == ARG_STAR:
        ...
        if not (
            ...
            return forward_base = reverse_type.arg_types[1]
    ...
    def check_inplace_operator_method(self, defn: FuncBase) -> None:
        ...
        if method not in operators.inplace_operator_methods:
            return typ = bind_self(self.function_type(defn))
        cls = defn.info
    ...
    def check_setattr_method(self, typ: Type, context: Context) -> None:
        if not self.scope.active_class():
            return method_type = CallableType(
            [
                AnyType(TypeOfAny.special_form),
                self.named_type("builtins.str"),
                AnyType(TypeOfAny.special_form),
    ...
    def check_match_args(self, var: Var, typ: Type, context: Context) -> None:
        ...
        if not self.scope.active_class():
            return typ = get_proper_type(typ)
    ...
    def check__exit__return_type(self, defn: FuncItem) -> None:
        ...
        if not defn.type or not isinstance(defn.type, CallableType):
            return ret_type = get_proper_type(defn.type.ret_type)
        if not has_bool_item(ret_type):
            return returns = all_return_statements(defn)
        if not returns:
            return if all(
    ...
    def try_infer_partial_generic_type_from_assignment(
        ...
        if var is not None:
            ...
            if typ.type is None:
                return # Return if this is an unsupported augmented assignment.
            if op != "=" and (typ.type.fullname, op) not in self.partial_type_augmented_ops:
                return # TODO: some logic here duplicates the None partial type counterpart
            #       inlined in check_assignment(), see #8043.
            partial_types = self.find_partial_types(var)
            if partial_types is None:
                return rvalue_type = self.expr_checker.accept(rvalue)
    ...
    def check_compatibility_all_supers(self, lvalue: RefExpr, rvalue: Expression) -> None:
        ...
        if (
            ...
            if not lvalue_type:
                ...
                return for base in lvalue_node.info.mro[1:]:
    ...
    def check_assignment_to_slots(self, lvalue: Lvalue) -> None:
        if not isinstance(lvalue, MemberExpr):
            return inst = get_proper_type(self.expr_checker.accept(lvalue.expr))
        if isinstance(inst, TypeVarType) and inst.id.is_self():
            # Unwrap self type
            inst = get_proper_type(inst.upper_bound)
        if not isinstance(inst, Instance):
            return if inst.type.slots is None:
    ...
    def try_infer_partial_type_from_indexed_assignment(
        ...
        if isinstance(var, Var):
            if isinstance(var.type, PartialType):
                ...
                partial_types = self.find_partial_types(var)
                if partial_types is None:
                    return typename = type_type.fullname
                if (
            ...
            if s.expr:
                ...
                if declared_none_return:
                    ...
                    if is_lambda or isinstance(typ, NoneType):
                        return self.fail(message_registry.NO_RETURN_VALUE_EXPECTED, s)
                else:
                    self.check_subtype(
                        subtype_label="got",
                        subtype=typ,
                ...
                if (
                    ...
                    return if isinstance(return_type, (NoneType, AnyType)):
                    return
    ...
    def visit_for_stmt(self, s: ForStmt) -> None:
        ...
        self.accept_loop(
            ...
            s.else_body,
            def on_enter_body():
                return self.analyze_index_variables(
    ...
    def visit_decorator_inner(
        ...
        if allow_empty or skip_first_item:
            return if e.func.info and not e.is_overload:
    ...
    def check_untyped_after_decorator(self, typ: Type, func: FuncDef) -> None:
        if not self.options.disallow_any_decorated or self.is_stub or self.current_node_deferred:
            return if mypy.checkexpr.has_any_type(typ):
    ...
    def check_for_truthy_type(self, t: Type, expr: Expression) -> None:
        ...
        if not self._is_truthy_type(t):
            return def format_expr_type() -> str:
    ...
    def refine_away_none_in_comparison(
        ...
        if not non_optional_types or (len(non_optional_types) != len(chain_indices)):
            ...
            # Narrow e.g. `Optional[A]  is not None` to `A` (which is stricter than the above step and
            # so type-safe but less convenient, because e.g. `Optional[A]  is None` still results
            # in `Optional[A]`):
    ...
    def check_possible_missing_await(
        ...
        with self.checking_await_set(), self.msg.filter_errors() as local_errors:
            ...
            if aw_type is None:
                return if not self.check_subtype(
        ...
        for other_variant in expand_callable_variants(other):
            ...
            if not (
                is_callable_compatible(
                    ...
                    is_proper_subtype=False,
                    def is_compat_return(l, r):
                        return not is_subset_no_promote(l, r),
                ...
                or is_callable_compatible(
                    ...
                    is_proper_subtype=False,
                    def is_compat_return(l, r):
                        return not is_subset_no_promote(r, l),
                    allow_partial_overlap=True,
                )
            ):
                continue
...
class DisjointDict(Generic[TKey, TValue]):
    ...
    def add_mapping(self, keys: set[TKey], values: set[TValue]) -> None:
        ...
        """
        if not keys:
            return subtree_roots = [self._lookup_or_make_root_id(key) for key in keys]
