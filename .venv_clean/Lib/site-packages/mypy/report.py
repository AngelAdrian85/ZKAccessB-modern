...
class XmlReporter(AbstractXmlReporter):
    ...
    def on_file(
        ...
        if last_xml is None:
            return path = os.path.relpath(tree.path)
        if path.startswith(".."):
            return out_path = os.path.join(self.output_dir, "xml", path + ".xml")
...
class XsltHtmlReporter(AbstractXmlReporter):
    ...
    def on_file(
        ...
        if last_xml is None:
            return path = os.path.relpath(tree.path)
        if path.startswith(".."):
            return out_path = os.path.join(self.output_dir, "html", path + ".html")
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
    ...
    * unanalyzed (mypy considers line unreachable)
    ...
    def on_file(
        ...
        if should_skip_path(path):
            ...
            return visitor = stats.StatisticsVisitor(
            inferred=True,
            filename=tree.fullname,
            modules=modules,
            typemap=type_map,
