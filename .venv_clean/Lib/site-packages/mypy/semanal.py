...
class SemanticAnalyzer(
    ...
    def visit_func_def(self, defn: FuncDef) -> None:
        ...
        if not self.recurse_into_functions:
            ...
            return with self.scope.function_scope(defn):
    ...
    def visit_overloaded_func_def(self, defn: OverloadedFuncDef) -> None:
        ...
        if not self.recurse_into_functions:
            ...
            return # NB: Since _visit_overloaded_func_def will call accept on the
    ...
    def process_deprecated_overload(self, defn: OverloadedFuncDef) -> None:
        if defn.is_property:
            return if isinstance(impl := defn.impl, Decorator) and (
    ...
    def process_overload_impl(self, defn: OverloadedFuncDef) -> None:
        ...
        if defn.impl is None:
            return impl = defn.impl if isinstance(defn.impl, FuncDef) else defn.impl.func
    ...
    def check_classvar_in_signature(self, typ: ProperType) -> None:
        ...
        if not isinstance(typ, CallableType):
            return for t in get_proper_types(typ.arg_types) + [get_proper_type(typ.ret_type)]:
    ...
    def analyze_class(self, defn: ClassDef) -> None:
        ...
        if self.analyze_namedtuple_classdef(defn, tvar_defs):
            ...
            return # Create TypeInfo for class now that base classes and the MRO can be calculated.
    ...
    def visit_assignment_expr(self, s: AssignmentExpr) -> None:
        ...
        if self.is_func_scope():
            if not self.check_valid_comprehension(s):
                return self.analyze_lvalue(s.target, escape_comprehensions=True, has_explicit_value=True)
    ...
    def visit_assignment_stmt(self, s: AssignmentStmt) -> None:
        ...
        if self.analyze_identity_global_assignment(s):
            return tag = self.track_incomplete_refs()
    ...
    def apply_dynamic_class_hook(self, s: AssignmentStmt) -> None:
        ...
        while True:
            ...
            break
        if not fname:
            return hook = self.plugin.get_dynamic_class_hook(fname)
        if not hook:
            return for lval in s.lvalues:
    ...
    def check_classvar(self, s: AssignmentStmt) -> None:
        ...
        if len(s.lvalues) != 1 or not isinstance(lvalue, RefExpr):
            return if not s.type or not self.is_classvar(s.type):
            return
    ...
    def visit_call_expr(self, expr: CallExpr) -> None:
        ...
        if refers_to_fullname(expr.callee, "typing.cast"):
            ...
            if not self.check_fixed_args(expr, 2, "cast"):
                return # Translate first argument to an unanalyzed type.
            try:
            ...
            if not self.check_fixed_args(expr, 2, "assert_type"):
                return # Translate second argument to an unanalyzed type.
            try:
            ...
            if not self.check_fixed_args(expr, 2, "divmod"):
                return expr.analyzed = OpExpr("divmod", expr.args[0], expr.args[1])
            expr.analyzed.line = expr.line
            expr.analyzed.accept(self)
        elif refers_to_fullname(
    ...
    def add_imported_symbol(
        ...
        if (
            ...
            if self.process_import_over_existing_name(name, existing_symbol, node, context):
                return symbol_node: SymbolNode | None = node.node
    ...
    def fail(
        ...
        *,
        ...
        if not serious and not self.in_checked_function():
            return # In case it's a bug and we don't really have context
