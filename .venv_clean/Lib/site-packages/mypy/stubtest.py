...
def _verify_static_class_methods(
    ...
    if static_runtime is MISSING:
        ...
        return if isinstance(static_runtime, classmethod) and not stub.is_class:
        yield "runtime is a classmethod but stub is not"
...
def _verify_arg_name(
    ...
    if runtime_arg.name in nonspecific_names:
        return def names_approx_match(a: str, b: str) -> bool:
        a = a.strip("_")
        b = b.strip("_")
    ...
    if runtime_arg.kind == inspect.Parameter.POSITIONAL_ONLY and names_approx_match(
        ...
        return # This comes up with namedtuples, so ignore
...
def verify_funcitem(
    ...
    if not is_probably_a_function(runtime):
        ...
        if not callable(runtime):
            return # Look the object up statically, to avoid binding by the descriptor protocol
    ...
    if not signature:
        ...
        return for message in _verify_signature(
        stub_sig,
        runtime_sig,
...
def verify_missing(
    ...
    if runtime is MISSING:
        return yield Error(object_path, "is not present in stub", stub, runtime)
...
def verify_overloadedfuncdef(
    ...
    if not is_probably_a_function(runtime):
        yield Error(object_path, "is not a function", stub, runtime)
        if not callable(runtime):
            return # mypy doesn't allow overloads where one overload is abstract but another isn't,
    ...
    signature = safe_inspect_signature(runtime)
    if not signature:
        return stub_sig = Signature.from_overloadedfuncdef(stub)
    runtime_sig = Signature.from_inspect_signature(signature)
...
def verify_typevarexpr(
    ...
    if isinstance(runtime, Missing):
        ...
        if stub.name == "_NT":
            return yield Error(object_path, "is not present at runtime", stub, runtime)
        return
...
def _verify_readonly_property(stub: nodes.Decorator, runtime: Any) -> Iterator[str]:
    ...
    if (
        ...
        return yield "is inconsistent, cannot reconcile @property on stub with runtime object"
...
def _verify_final_method(
    ...
    if stub.is_final:
        return if getattr(runtime, "__final__", False) or (
...
def verify_typealias(
    ...
    if isinstance(stub_target, mypy.types.AnyType):
        return yield Error(object_path, "is not a recognised type alias", stub, runtime, stub_desc=stub_desc)
