...
class SQLDiff:
    ...
    def SQL_NOTNULL_DIFFER(self, style, qn, args):
        return "%s %s\n\t%s %s %s %s;" % (
        style.SQL_KEYWORD("ALTER TABLE"),
        style.SQL_TABLE(qn(args[0])),
        style.SQL_KEYWORD("MODIFY"),
    ...
    def SQL_ERROR(self, style, qn, args):
        return style.NOTICE(
    ...
    def SQL_TABLE_MISSING_IN_MODEL(self, style, qn, args):
        return style.NOTICE(
    ...
    def find_field_notnull_differ(self, meta, table_description, table_name):
        if not self.can_detect_notnull_differ:
            return for field in all_local_fields(meta):
            attname = field.db_column or field.attname
