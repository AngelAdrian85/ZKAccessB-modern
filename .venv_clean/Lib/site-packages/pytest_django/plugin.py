...
def pytest_load_initial_conftests(
    ...
    if options.version or options.help:
        ...
        return django_find_project = _get_boolean_value(
...
def pytest_configure(config: pytest.Config) -> None:
    if config.getoption("version", 0) > 0 or config.getoption("help", False):
        return # Normally Django is set up in `pytest_load_initial_conftests`, but we also
...
def pytest_collectstart(collector: pytest.Collector) -> None:
    if "django" not in sys.modules:
        return if not isinstance(collector, pytest.Class):
        return

    tags = getattr(collector.obj, "tags", ())
    if not tags:
        return from django.test import SimpleTestCase
...
def pytest_itemcollected(item: pytest.Item) -> None:
    if "django" not in sys.modules:
        return if not isinstance(item, pytest.Function):
        return
...
def pytest_collection_modifyitems(items: list[pytest.Item]) -> None:
    ...
    if not django_settings_is_configured():
        return from django.test import TestCase, TransactionTestCase
...
def _dj_autoclear_mailbox() -> None:
    if not django_settings_is_configured():
        return from django.core import mail
