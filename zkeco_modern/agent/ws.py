...
def broadcast_device_status(device_id: int, online: bool, door_state: str = None, serial: str = None, updated_at: str = None):
    ...
    try:
        ...
        # Persist status transitions (best-effort) so server DB has history.
        # Do not rely on DeviceStatus.online, because callers may update it before broadcasting.
        try:
            ...
            try:
                last = (
                    AuditLog.objects.filter(module='device-status', entity_id=int(device_id))
                    .only('action', 'timestamp')
                    .order_by('-timestamp')
                    .first()
                )
                if last is not None:
                    if str(getattr(last, 'action', '') or '') == 'online':
                        prev = True
                    elif str(getattr(last, 'action', '') or '') == 'offline':
                        prev = False
            except Exception:
                prev = None

            if prev is None or prev != bool(online):
                AuditLog.objects.create(
                    module='device-status',
                    action='online' if bool(online) else 'offline',
                    entity_id=int(device_id),
                    entity_name=(serial or f"device_id={int(device_id)}"),
                    user=None,
                    ip_address=None,
                    details=json.dumps({
                        'online': bool(online),
