...
function setTime($input,tmp_time){
	var dSplit=$.trim($input.val());
	//自定义时间格式---huangjs20120417
	if(dSplit.indexOf("-")==-1 && dSplit.indexOf("/")==-1){
		dSplit="";
	}else{
...
{
	if($("html").attr("lang") != "zh-cn" && $("html").attr("lang") != "zh-tw"){
	...
	{	
		$obj.hide();
		//模拟下拉框的全部脚本
		var html = "<div class='dropdown_box'>\
		...
		$(document).click(function(event){
			try
			{
				var element_class = event.srcElement.className;
			}
			catch(e)
			{
				try
				...
				{
					var element_class = "";//避免其他特殊页面影响（比如监控页面）。
				}
		...
		//根据select生成一个模拟下拉框
		//var tmp = false;//避免重复加载option-darcy20120529
		//if($(".dropdown_box_list div").length == 0)
		//{
		//	tmp = true;
		//}
