 ...
 /*******基于模型的datalist开始,正对前面代码的一个应用实例********/
    ...
    $.fn.model_grid=function(opt){
        ...
        var g ={
            ...
            max_no_page: 15,                 //记录数少于该数据时，不分页显示
                ...
                if(g.obj_edit){
                   /* g.on_dbl_click=function(grid,index,key,row_data,e){
                    ...
                    g.on_row_href=function(grid,index,key,row_data,e){
                        var href=grid.g.model_url+key+"/";
                        //处理同一个模型多处使用的情况---darcy20120424
                        href = href+"?virtual_app="+$("#id_sys_virtual_app").val();
                        //alert(href)
                        $.zk._processEdit(href, grid, undefined, e);
                        return false;
                    };
                        ...
                        {   
                            ...
                            if(!is_reload){
                                ...
                                try
                                {
                                    ...
                                    //--start 可自定义查询结束数据载入页面后需要的操作---by liangm 20120821
                                     if(typeof(after_query_load)!= "undefined")
                                     {
                                        if(!after_query_load()){
                                            return;
                                        };
                                ...
                                {
                                    //alert(exp);//点的过快时会导致对象错误,重新刷新页面即可---darcy20120528.
                                }
                            }
                           // g.sort_data();
                        }
                    ...
                    var op_edit={
                        op_edit:{
                            ...
                            on_click:function(grid,selected_objs,event,elem)
                            {
                                if(elem)
                                {
                                    var href=grid.g.model_url+$(elem).attr("data")+"/"+"?virtual_app="+$("#id_sys_virtual_app").val();//处理一个模型多个页面使用的情况。-darcy20120425
                                    $.zk._processEdit(href, grid, undefined, event);
                                }
                                return false;
                            }
