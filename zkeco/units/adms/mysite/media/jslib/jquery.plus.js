 ...
 * @author Kelvin Luck
 */
var date_format = "mm/dd/yyyy";//初始化值，获取失败默认用这种格式
var surl=$("#id_surl").val();
$.ajax({
    type: "POST",
    url: '/'+surl+'data/GetDateFormat',
    async: false,
    dataType: "json",
    success: function(datas){
        date_format = datas.ret;
    }
})
		...
		renderCalendar  :   function(s)
		{
			...
			while (w++<weeksToDraw) {
				...
				for (var i=0; i<7; i++) {
					...
					r.append(d);
					if(currentDate.getDate() == currentDate.addDays(1).getDate())//win7英文32位系统下，每年的11月部分日期会重复，需要过滤 ---by liangm 20121114
					{
	...
	$.extend(
		...
		{	
			...
			setDisplayedMonth : function(m, y)
			{
				...
				//去掉比较，格式不是为“yyyy-mm-dd”时，可以选择当前时间前的日期(年、月)----huangjs
...
//				}
			...
			_renderCalendar : function()
			{
				...
				//去掉判断，不是“yyyy-mm-dd”格式时，可以取日期（天）--huangjs
...
(function($){$.extend({progressBar:new function(){this.defaults={increment:2,speed:15,showText:true,width:119,boxImage:'/media/images/progressbar.gif',barImage:{0:'/media/images/progressbg_red.gif',30:'/media/images/progressbg_orange.gif',70:'/media/images/progressbg_green.gif'},height:12};this.construct=function(arg1,arg2){var argpercentage=null;var argconfig=null;if(arg1!=null){if(!isNaN(arg1)){argpercentage=arg1;if(arg2!=null){argconfig=arg2;}}else{argconfig=arg1;}}
