...
function init_login(
    ...
    $user_pwd.keydown(function(event){
        ...
        if(event.keyCode == 9){
            $login.focus();
            $login.parent().parent().attr("class","login_btn1");
            if(event.preventDefault)
            {
                event.preventDefault();
            }
    ...
    $reset.keydown(function(event){
        ...
        if(event.keyCode == 9)
        {
            if($('body').find('#'+finger_login_id).length == 0 || $finger_login.attr('disabled'))
            {
                $user_name.focus();
            }
            else
            {
                $finger_login.focus();
                $finger_login.parent().parent().attr("class","login_btn1");
            }
            $reset.parent().parent().attr("class","login_btn0");
        }
        if(event.preventDefault)
        {
            event.preventDefault();
    ...
    $login.keydown(function(event){
        ...
        if(event.keyCode == 9){
            $reset.focus();
            $reset.parent().parent().attr("class","login_btn1");
            $login.parent().parent().attr("class","login_btn0");
            if(event.preventDefault)
            {
                event.preventDefault();
            }
    ...
    $finger_login.keydown(function(event){
        if(event.keyCode == 13)
        {
            $finger_login.click();
        }
        if(event.keyCode == 9)
        {
            ...
            event.returnValue=false;
            $finger_login.parent().parent().attr("class","login_btn0");
            $user_name.focus();                    
        }
    });
...
//        }
        ...
        else if($("#id_client_language").val().toLowerCase() == 'es')
        {
            zkonline.SetLanguageFile("zkonline.es");  
        }
        else if($("#id_client_language").val().toLowerCase() == 'zh-tw')
        {
        ...
        if(zkonline.GetVerTemplate()){
            ...
            $.ajax({
                ...
                success:function(msg){
                    ...
                    else if(msg == gettext("已过软件试用期，请购买许可！"))
                    {		
                        $(".messageBox").html(gettext("已过软件试用期，请购买许可！")).show();
                        return false;
                    }
                    else 
                    {
                        $(".messageBox_").html(gettext("验证通过，登录系统!")).show();
                        window.location.href=redirect_url;
                    }
                }
            });                      
    ...
    //if(!$.browser.msie){
	if(!isIE()){	
        $finger_login.attr({disabled:"true", title: gettext("登记指纹功能只支持IE浏览器")}).css({cursor:"default",color:"#888888"}).unbind().click(function(){
        ...
        if(!flag){
            $finger_login.attr({disabled:"true", title: gettext("请安装指纹仪驱动")}).css({cursor:"default",color:"#888888"}).unbind().click(function(){
                return false;
            });
        }
    }
...
$(function(){
    ...
    $('body').click(function(){
        $(this).find('.login_btn1').attr('class','login_btn0');
    });
    ...
    //员工自助登入
    init_login(false,'id_username_self','id_password_self','id_login_self','login-self-form','id_reset_self','/selfservice/login/','/selfservice/reservation/','id_fp_identify_self')
});