...
# optparse removed; use argparse via BaseCommand.add_arguments
...
def copy_plural_forms(msgs, locale, domain, verbosity):
    ...
    for domain in domains:
        ...
        if os.path.exists(django_po):
            # open with explicit encoding under Python 3
            with open(django_po, 'r', encoding='utf-8') as f:
...
def make_messages(locale=None, domain='django', verbosity='1', all=False,
    ...
    if locale is not None:
        ...
        languages = [os.path.basename(dirname) for dirname in locale_dirs]
    ...
    for locale in languages:
        ...
        for dirpath, file in find_files('.', ignore_patterns, verbosity, symlinks=symlinks) + \
                find_files('D:/trunk/python-support/base', ignore_patterns, verbosity, symlinks=symlinks) + \
                find_files('D:/trunk/python-support/dbapp', ignore_patterns, verbosity, symlinks=symlinks):
            file_base, file_ext = os.path.splitext(file)
            if domain == 'djangojs' and file_ext in extensions:
                if verbosity > 1:
                    sys.stdout.write('processing file %s in %s\n' % (file, dirpath))
                # read non-Python files as text
                with open(os.path.join(dirpath, file), 'r', encoding='utf-8', errors='replace') as f:
                    src = f.read()
                src = pythonize_re.sub('\n#', src)
                thefile = '%s.py' % file
                with open(os.path.join(dirpath, thefile), 'w', encoding='utf-8') as f:
                    f.write(src)
                ...
                if msgs:
                    # write as text under Python 3
                    with open(potfile, 'a', encoding='utf-8') as f:
                        f.write(msgs)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--locale', '-l', default=None, dest='locale',
        ...
        parser.add_argument('--extension', '-e', dest='extensions', action='append',
    ...
    def handle(self, *args, **options):
        ...
        ignore_patterns = options.get('ignore_patterns') or []
