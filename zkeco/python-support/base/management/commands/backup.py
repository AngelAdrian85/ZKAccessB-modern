...
def backupdb():
    ...
    for i in pos:
        ...
        if hl.count()>0:
            ...
            dt_start =starttime
        k = 1
        dd = datetime.datetime(currenttime.year, currenttime.month, currenttime.day,
                               dt_start.hour, dt_start.minute, dt_start.second)
        # iterate future scheduled times until we either find a matching window
        # or determine the next scheduled time is after currenttime
        while True:
            ...
            if currenttime < dt_start_from:
                # next scheduled time is in the future; nothing to do
                break
            if dt_start_from <= currenttime <= dt_start_to:
                # time to schedule a backup record
                iCount = DbBackupLog.objects.filter(user=i.user, imflag=False,
    ...
    if cunprocess:
        ...
        try:
            # Use current Python executable to run manage.py dumpdata for portability
            cmd = '"{0}" manage.py dumpdata > "{1}"'.format(sys.executable, backup_file)
            ...
            # Log full exception
            logger.exception("Error running dumpdata: %s", e)
