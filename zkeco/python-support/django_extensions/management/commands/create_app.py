...
from django.core.management.base import CommandError, BaseCommand, _make_writeable
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('label', help='application name')
        parser.add_argument('--template', '-t', action='store', dest='app_template', help='The path to the app template')
        parser.add_argument('--parent_path', '-p', action='store', dest='parent_path', help='The parent path of the application to be created')
        parser.add_argument('-d', action='store_true', dest='dia_parse', help='Generate model.py and admin.py from [APP_NAME].dia file')
        parser.add_argument('--diagram', action='store', dest='dia_path', help='The diagram path of the app to be created. -d is implied')

    def handle(self, label=None, **options):
    ...
    def handle_label(self, label, **options):
        ...
        if dia_parse:
            ...
            print(
                "Please add now %r and any other dependent application in "
    ...
    for d, subdirs, files in os.walk(app_template):
        ...
        for f in files:
            ...
            try:
                ...
                # Fallback: write a simple notice to stderr rather than relying on style utility
