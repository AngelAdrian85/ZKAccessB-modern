...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write("Session Key does not exist. Expired?")
            return

        data = session.get_decoded()
        self.stdout.write('Session to Expire: %s' % session.expire_date)
        self.stdout.write('Raw Data: %r' % (data,))
        uid = data.get('_auth_user_id', None)
        if uid is None:
            self.stdout.write('No user associated with session')
            return
        self.stdout.write('User id: %s' % uid)
        try:
            user = User.objects.get(pk=uid)
        except User.DoesNotExist:
            self.stdout.write("No user associated with that id.")
            return
        for key in ['username', 'email', 'first_name', 'last_name']:
            self.stdout.write('%s: %s' % (key, getattr(user, key)))
