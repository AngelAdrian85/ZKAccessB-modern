...
class Command(BaseCommand):
    help = (
        ...
        'Possible parameters are: username, first_name, last_name' % (DEFAULT_FAKE_EMAIL,)
    )
    requires_model_validation = False

    def add_arguments(self, parser):
        parser.add_argument('--email', dest='default_email', default=DEFAULT_FAKE_EMAIL,
                            help='Use this as the new email format.')
        parser.add_argument('-a', '--no-admin', action='store_true', dest='no_admin', default=False,
                            help='Do not change administrator accounts')
        parser.add_argument('-s', '--no-staff', action='store_true', dest='no_staff', default=False,
        ...
        parser.add_argument('--include-groups', dest='include_groups', default=None,
                            help='Include users matching this group. (use comma seperation for multiple groups)')
        parser.add_argument('--exclude-groups', dest='exclude_groups', default=None,
    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            user.save()
        self.stdout.write('Changed %d emails' % users.count())
