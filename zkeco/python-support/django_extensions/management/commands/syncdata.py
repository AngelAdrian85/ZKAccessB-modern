    ...
    def remove_objects_not_in(self, objects_to_keep, verbosity):
        ...
        for class_ in objects_to_keep.keys():
            ...
            if remove_these_ones:
                ...
                for obj in current:
                    if obj.id in remove_these_ones:
                        obj.delete()
                        if verbosity >= 2:
                            self.stdout.write("Deleted object: " + str(obj))
            ...
            if verbosity > 0 and remove_these_ones:
                ...
                self.stdout.write("Deleted " + str(num_deleted) + " " + type_deleted)
    ...
    def handle(self, *fixture_labels, **options):
        ...
        for fixture_label in fixture_labels:
            ...
            if formats:
                ...
                sys.stderr.write(
                    self.style.ERROR("Problem installing fixture '%s': %s is not a known serialization format." % (fixture_name, format))
                )
            ...
            for fixture_dir in fixture_dirs:
                ...
                for format in formats:
                    # obtain serializer class (not used directly here) - keep for compatibility
                    _serializer = serializers.get_serializer(format)
                    if verbosity > 1:
        ...
        if object_count == 0:
            ...
            if verbosity > 0:
                self.stdout.write("Installed %d object(s) from %d fixture(s)" % (object_count, fixture_count))
...
# optparse compatibility removed; Django provides --verbosity by default
