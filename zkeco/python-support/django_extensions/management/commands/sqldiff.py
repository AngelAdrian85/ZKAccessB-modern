...
def flatten(seq, ltypes=(list, tuple)):
    """Flatten nested lists/tuples one level deep while preserving the
    original sequence type.

    This mirrors the original behaviour but uses clearer variable names to
    avoid ambiguous single-letter identifiers that linters flag (E741).
    """
    seq_type = type(seq)
    seq_list = list(seq)
    idx = 0
    while idx < len(seq_list):
        while isinstance(seq_list[idx], ltypes):
            if not seq_list[idx]:
                seq_list.pop(idx)
                idx -= 1
                break
            else:
                seq_list[idx: idx + 1] = seq_list[idx]
        idx += 1
    return seq_type(seq_list)
...
class SQLDiff(object):
    ...
    def SQL_FIELD_MISSING_IN_DB(self, style, qn, args):
        return "%s %s\n\t%s %s %s;" % (
            style.SQL_KEYWORD('ALTER TABLE'),
            style.SQL_TABLE(qn(args[0])),
            style.SQL_KEYWORD('ADD'),
            style.SQL_FIELD(qn(args[1])),
            style.SQL_COLTYPE(args[2]),
        )

    def SQL_FIELD_MISSING_IN_MODEL(self, style, qn, args):
        return "%s %s\n\t%s %s;" % (
            style.SQL_KEYWORD('ALTER TABLE'),
            style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_INDEX_MISSING_IN_DB(self, style, qn, args):
        return "%s %s\n\t%s %s (%s);" % (
            style.SQL_KEYWORD('CREATE INDEX'),
            style.SQL_TABLE(qn("%s_idx" % '_'.join(args[0:2]))),
    ...
    def SQL_INDEX_MISSING_IN_MODEL(self, style, qn, args):
        return "%s %s;" % (
            style.SQL_KEYWORD('DROP INDEX'),
            style.SQL_TABLE(qn("%s_idx" % '_'.join(args[0:2]))),
        )

    def SQL_UNIQUE_MISSING_IN_DB(self, style, qn, args):
        return "%s %s\n\t%s %s (%s);" % (
            style.SQL_KEYWORD('ALTER TABLE'),
            style.SQL_TABLE(qn(args[0])),
            style.SQL_KEYWORD('ADD'),
    ...
    def SQL_UNIQUE_MISSING_IN_MODEL(self, style, qn, args):
        return "%s %s\n\t%s %s %s;" % (
            style.SQL_KEYWORD('ALTER TABLE'),
            style.SQL_TABLE(qn(args[0])),
            style.SQL_KEYWORD('DROP'),
            style.SQL_KEYWORD('CONSTRAINT'),
            style.SQL_TABLE(qn("%s_key" % ('_'.join(args[:2])))),
        )

    def SQL_FIELD_TYPE_DIFFER(self, style, qn, args):
        return "%s %s\n\t%s %s %s;" % (
            style.SQL_KEYWORD('ALTER TABLE'),
            style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_FIELD_PARAMETER_DIFFER(self, style, qn, args):
        return "%s %s\n\t%s %s %s;" % (
            style.SQL_KEYWORD('ALTER TABLE'),
            style.SQL_TABLE(qn(args[0])),
            style.SQL_KEYWORD("MODIFY"),
    ...
    def SQL_COMMENT(self, style, qn, args):
        return style.NOTICE('-- Comment: %s' % style.SQL_TABLE(args[0]))

    def SQL_TABLE_MISSING_IN_DB(self, style, qn, args):
        return style.NOTICE('-- Table missing: %s' % args[0])
    ...
    def find_unique_missing_in_model(self, meta, table_indexes, table_name):
        ...
        for att_name, att_opts in table_indexes.items():
        ...
        for app_label, model_name, diffs in self.differences:
            ...
            for diff in diffs:
                ...
                if not self.dense:
                    ...
                    # use app_label (not previously defined app_name)
...
class MySQLDiff(SQLDiff):
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if not db_type:
            # No DB type discovered from introspection; if we don't have
            # a model field to consult, return None.
            if not field:
                return db_type
            # Obtain the model field type and attempt conservative fixes
            field_type = self.get_field_model_type(field)
            if field.primary_key and (field_type == 'integer' or field_type == 'bigint'):
                db_type = field_type + ' AUTO_INCREMENT'
            # Fix char/varchar inconsistencies
            if self.strip_parameters(field_type) == 'char' and self.strip_parameters(db_type or '') == 'varchar':
                db_type = (db_type or '').lstrip("var")
            # They like to call 'bool's 'tinyint(1)' and introspection makes that an integer
            if (db_type or '') == 'integer' and description[1] == FIELD_TYPE.TINY and description[4] == 1:
                db_type = 'bool'
            if (db_type or '') == 'integer' and description[1] == FIELD_TYPE.SHORT:
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if not db_type:
            if not field:
                return db_type
            field_type = self.get_field_model_type(field)
            # If introspection failed, fall back to the model field type
            db_type = field_type
            # Fix char/varchar inconsistencies
...
class PostgresqlSQLDiff(SQLDiff):
    ...
    def SQL_FIELD_TYPE_DIFFER(self, style, qn, args):
        """Return ALTER TABLE ... ALTER <field> TYPE <coltype>; for Postgres."""
        return (
            "%s %s\n\t%s %s %s %s;" % (
                style.SQL_KEYWORD('ALTER TABLE'),
                style.SQL_TABLE(qn(args[0])),
    ...
    def SQL_FIELD_PARAMETER_DIFFER(self, style, qn, args):
        """Alias for SQL_FIELD_TYPE_DIFFER; kept for compatibility."""
        return self.SQL_FIELD_TYPE_DIFFER(style, qn, args)
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if not db_type:
            if not field:
                return db_type
            # Heuristic: if it's a primary key of integer type on the model,
            # treat it as serial in Postgres.
            try:
                field_type = self.get_field_model_type(field)
            except Exception:
                field_type = None
            if field.primary_key and field_type == 'integer':
                db_type = 'serial'
            if table_name:
                ...
                if check_constraint:
                    ...
                    db_type = (db_type or '') + ' ' + check_constraint
        return db_type
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument('--not-only-existing', '-e', action='store_false', dest='only_existing',
                            help="Check all tables that exist in the database, not only tables that should exist based on models.")
        parser.add_argument('--dense-output', '-d', action='store_true', dest='dense_output',
                            help="Shows the output in dense format, normally output is spread over multiple lines.")
        parser.add_argument('--output_text', '-t', action='store_false', dest='sql', default=True,
