...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument('--model', action='append', dest='added_model_list', type=str,
        ...
        parser.add_argument('--add-field', action='append', dest='added_field_list', type=str,
                            help='Generate an Add Column migration for the specified modelname.fieldname - you can use this multiple times to add more than one column.')
        parser.add_argument('--add-index', action='append', dest='added_index_list', type=str,
        ...
        parser.add_argument('--initial', action='store_true', dest='initial', default=False,
        ...
        parser.add_argument('--freeze', action='append', dest='freeze_list', type=str,
    ...
    def handle(self, app=None, name="", added_model_list=None, added_field_list=None, initial=False, freeze_list=None, auto=False, stdout=False, added_index_list=None, **options):
        ...
        if initial and (added_model_list or added_field_list or auto):
            self.stdout.write("You cannot use --initial and other options together")
            self.stdout.write(self.usage_str)
            return
        if auto and (added_model_list or added_field_list or initial):
            self.stdout.write("You cannot use --auto and other options together")
            self.stdout.write(self.usage_str)
            return
        ...
        if not name:
            self.stdout.write("You must name this migration")
            self.stdout.write(self.usage_str)
            return
        
        if not app:
            self.stdout.write("Please provide an app in which to create the migration.")
            self.stdout.write(self.usage_str)
            return
        ...
        if not app_models_module:
            self.stdout.write("App '%s' doesn't seem to exist, isn't in INSTALLED_APPS, or has no models." % app)
            self.stdout.write(self.usage_str)
            return
        ...
        try:
            ...
            self.stdout.write("Couldn't find path to App '%s'." % app)
            self.stdout.write(self.usage_str)
            return
        ...
        if not os.path.isdir(migrations_dir):
            self.stdout.write("Creating migrations directory at '%s'..." % migrations_dir)
            os.mkdir(migrations_dir)
        init_path = os.path.join(migrations_dir, "__init__.py")
        if not os.path.isfile(init_path):
            # Touch the init py file
            self.stdout.write("Creating __init__.py in '%s'..." % migrations_dir)
        ...
        new_filename = "%04i%s_%s.py" % (
            highest_number + 1,
            "",  # random insertion removed (was using string.letters which is Python2-only)
            name,
        )
        ...
        for fielddef in added_field_list:
            try:
                ...
                self.stdout.write("The field specification '%s' is not in modelname.fieldname format." % fielddef)
            else:
        ...
        if freeze_list:
            for item in freeze_list:
                if "." in item:
                    ...
                    if model is None:
                        self.stdout.write("Cannot find the model '%s' to freeze it." % item)
                        self.stdout.write(self.usage_str)
                        return
        ...
        if auto:
            ...
            if app_module is None:
                self.stdout.write("You cannot use automatic detection on the first migration of an app. Try --initial instead.")
            else:
                ...
                if not migrations:
                    ...
                    if hasattr(migrations[-1], "complete_apps") and \
                       app in migrations[-1].complete_apps:
                        ...
                        self.stdout.write("You cannot use automatic detection, since the previous migration does not have this whole app frozen.\nEither make migrations using '--freeze %s' or set 'SOUTH_AUTO_FREEZE_APP = True' in your settings.py." % app)
            ...
            if not (am or dm or af or df or cf or afu or dfu or was_meta_change):
                self.stdout.write("Nothing seems to have changed.")
                return
        ...
        for mkey in added_models:
            ...
            self.stdout.write(" + Added model '%s'" % (mkey,))
        ...
        for mkey, field_name in added_fields:
            ...
            try:
                ...
                self.stdout.write("Model '%s' doesn't have a field '%s'" % (mkey, field_name))
                return
            ...
            if isinstance(field, models.ManyToManyField):
                if not field.rel.through: # Bug #120
                    ...
                    self.stdout.write(" + Added M2M '%s.%s'" % (mkey, field_name))
                continue
            ...
            self.stdout.write(" + Added field '%s.%s'" % (mkey, field_name))
            ...
            _field_definition = make_field_constructor(app, field, triple)
        ...
        for mkey, field_name, field, triple, last_models in deleted_fields:
            ...
            self.stdout.write(" - Deleted field '%s.%s'" % (mkey, field_name))
        ...
        for model, fields, last_models in deleted_models:
            ...
            self.stdout.write(" - Deleted model '%s.%s'" % (model._meta.app_label,model._meta.object_name))
        ...
        for mkey, field_name in added_indexes:
            ...
            if field.unique:
                ...
                self.stdout.write(" + Added index for '%s.%s'" % (mkey, field_name))

            else:
                self.stdout.write("Field '%s.%s' does not have db_index or unique set to True" % (mkey, field_name))
                return
        ...
        for mkey, field_name, old_triple, new_triple, last_orm in changed_fields:
            ...
            if hasattr(field, "m2m_db_table"):
                ...
                if old_triple[1] != new_triple[1]:
                    self.stdout.write(" ! Detected change to the target model of M2M field '%s.%s'. South can't handle this; leaving this change out." % (mkey, field_name))
                continue
            
            self.stdout.write(" ~ Changed field '%s.%s'." % (mkey, field_name))
        ...
        for mkey, ut in added_uniques:
            ...
            model = model_unkey(mkey)
            if len(ut) == 1:
                self.stdout.write(" + Added unique for %s on %s." % (", ".join(ut), model._meta.object_name))
            else:
                self.stdout.write(" + Added unique_together for [%s] on %s." % (", ".join(ut), model._meta.object_name))
        ...
        for mkey, ut, model in deleted_uniques:
            ...
            if len(ut) == 1:
                self.stdout.write(" - Deleted unique for %s on %s." % (", ".join(ut), model._meta.object_name))
            else:
                self.stdout.write(" - Deleted unique_together for [%s] on %s." % (", ".join(ut), model._meta.object_name))
        ...
        for modelname, model in all_models.items():
            for fieldname, fielddef in model.items():
                ...
                # Warn about undefined fields
                    ...
                    sys.stderr.write("WARNING: Cannot get definition for '%s' on '%s'. Please edit the migration manually to define it, or add the south_field_triple method to it.\n" % (fieldname, modelname))
        ...
        if name == "-":
            self.stdout.write(file_contents)
        # Write the migration file if the name isn't -
        else:
            fp = open(os.path.join(migrations_dir, new_filename), "w")
            fp.write(file_contents)
            fp.close()
            self.stdout.write("Created %s." % new_filename)
...
def model_unkey(key):
    ...
    if not model:
        sys.stderr.write("Couldn't find model '%s' in app '%s'\n" % (modelname, app))
        sys.exit(1)
    return model
...
def poss_ormise(default_app, rel_to, arg):
    ...
    for quote_type in QUOTES:
        ...
        if arg[:quote_len] == quote_type and arg[-quote_len:] == quote_type:
            arg = arg[quote_len:-quote_len]
            break
...
def triples_to_defs(app, model, fields):
    ...
    for field, triple in fields.items():
        triple = remove_useless_attributes(triple)
        if triple is None:
            sys.stderr.write("WARNING: Cannot get definition for '%s' on '%s'. Please edit the migration manually.\n" % (field, model_key(model)))
